			...
			for(let i = 0; i < movie.length; i++){
				...
				if(favoriteMovies.indexOf(id) === -1){
					...
					<div class="card">
						<div class="overlay">
						<div class="addBtn">
						<span><i class="fa fa-list plan" onclick="ptw('${movie[i].id}')"></i></span>
						<span><i class="material-icons watch" onclick="completed('${movie[i].id}')">visibility</i></span>
			...
			for(let i = 0; i < movie.length; i++){
				...
				if(favoriteMovies.indexOf(id) === -1){
					...
					<div class="card">
						...
						<span><i class="fa fa-list plan" onclick="ptw('${movie[i].id}')"></i></span>
						<span><i class="material-icons watch" onclick="completed('${movie[i].id}')">visibility</i></span>
						<span><i class="material-icons favorite" onclick="favorite('${movie[i].id}')">favorite</i></span>
						</div>
						<div class="movie">
							<h2>${movie[i].title}</h2>
								<p id="p_rating"><strong>Rating:</strong> <span>${movie[i].vote_average} / 10  <i class="material-icons star">star_rate</i></span> </p>
...
//PTW
function ptw(id){
    let storedId = JSON.parse(localStorage.getItem("ptwMovies")) || [];
	if(storedId.indexOf(id) === -1){
		storedId.push(id);
		localStorage.setItem("ptwMovies", JSON.stringify(storedId));
		//Notification that it will be added to Watchlist.
        const added = document.getElementById("added");
...
function completed(id){
	...
	let ptwstoredId = JSON.parse(localStorage.getItem("ptwMovies")) ||  [];
	if(storedId.indexOf(id) === -1){
		storedId.push(id);
		...
		for(let i = 0; i < ptwstoredId.length; i++) {
			if (id==ptwstoredId[i])	{
    			...
    			localStorage.setItem("ptwMovies", JSON.stringify(ptwstoredId));
			}
		}
		//Notification that it will be added to Watchlist.
        const added = document.getElementById("added");
