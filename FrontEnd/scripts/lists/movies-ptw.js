...
window.onload=function displayMovieWatchlist(){
    // MOVIES
    let toWatch = JSON.parse(localStorage.getItem("ptwMovies")) || [];
    ...
    if(toWatch.length == 0) {
        ...
        movieOutput.innerHTML +=
        `<p class="infoText"> No movies under "Plan to Watch" . Add some. <a href="#" onclick="openRecommendMoviesBox()"> Here are some recommendations !</a> </p>`;
    }
}
...
removeAllMovies.addEventListener("click", ()=>{
    localStorage.removeItem("ptwMovies");
