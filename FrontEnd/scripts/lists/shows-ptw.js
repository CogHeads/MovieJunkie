...
window.onload=function displayTVWatchlist(){
    // TV SHOWS
    let toWatchTvShows = JSON.parse(localStorage.getItem("ptwSeries")) || [];
    ...
    if(toWatchTvShows == 0){
        ...
        TvShowsOutput.innerHTML +=
        `<p class="infoText"> No shows under "Plan to Watch". Add some. <a href="#" onclick="openRecommendTvShowsBox()"> Here are some recommendations !</a> </p>`;
    }
}
// Recommend tv shows.
...
removeAllTvShows.addEventListener("click", ()=>{
    localStorage.removeItem("ptwSeries");
