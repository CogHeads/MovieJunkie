		...
		.then( ([seriesResponse, imdbResponse]) =>{
			...
			<div class="moviePage">
					...
					<div class="info">
						...
						<div class="buttons">
						<p class="addto"> Add to: </p>
							<a onclick="ptw('${movie.id}')"> Plan to Watch </a>
							<a onclick="completed('${movie.id}')"> Completed </a>
							<a onclick="favorite('${movie.id}')"> Favorites </a>
						</div>
					</div>
				</div>
				<div class="plot">
		...
		.then( ([seriesResponse, imdbResponse, seriesCastResponse]) =>{
			...
			<div class="moviePage">
					...
					<div class="info">
						...
						<div class="buttons">
							<p class="addto"> Add to: </p>
							<a onclick="ptw('${series.id}')"> Plan to Watch </a>
							<a onclick="completed('${series.id}')"> Completed </a>
							<a onclick="favorite('${series.id}')"> Favorites </a>
						</div>
					</div>
				</div>
				<div class="plot">
...
//PTW
function ptw(id){
    let storedId = JSON.parse(localStorage.getItem("ptwSeries")) || [];
	if(storedId.indexOf(id) === -1){
		storedId.push(id);
		localStorage.setItem("ptwSeries", JSON.stringify(storedId));
		//Notification that it will be added to Watchlist.
        const added = document.getElementById("added");
        added.innerHTML = "Added to Plan To Watch !";
        ...
        alreadyStored.innerHTML = "Already in Plan To Watch !";
...
function completed(id){
	...
	let ptwstoredId = JSON.parse(localStorage.getItem("ptwSeries")) ||  [];
	if(storedId.indexOf(id) === -1){
		storedId.push(id);
		localStorage.setItem("series", JSON.stringify(storedId));
		let index = ptwstoredId.indexOf(id);
		for(let i = 0; i < ptwstoredId.length; i++) {
			if (id==ptwstoredId[i])	{
				ptwstoredId.splice(index, 1);
    			localStorage.setItem("ptwSeries", JSON.stringify(ptwstoredId));
			}
		}
		//Notification that it will be added to Watchlist.
        const added = document.getElementById("added");
        added.innerHTML = "Added to Completed !"
        added.classList.add("added");
        setTimeout(() => {
            added.classList.remove("added");
        }, 1500);
		...
		//Notification that it has already been added to the watchlist.
        const alreadyStored = document.getElementById("alreadyStored");
        alreadyStored.innerHTML = "Already in Completed !"
        alreadyStored.classList.add("alreadyStored");
        setTimeout(() => {
            alreadyStored.classList.remove("alreadyStored");
        }, 1500);
