...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_id_err) && empty($password_err)){
        ...
        $sql = "SELECT user_id, email_id, password FROM database_project WHERE email_id = ?";
        
        if($stmt = mysqli_prepare($DB_Manager->GetLink(), $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    $email_id_err = "No account found with that email.";
                }
            } else{
    ...
    // Close connection
    mysqli_close($DB_Manager->GetLink());
}
?>
